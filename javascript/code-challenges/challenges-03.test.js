...
const typeNum = (arr) => {
  let solution = arr.filter((a) => typeof a === 'number');
  return solution;
};
...
const containsAnd = (arr) => {
  let solution = arr.filter((a) => a.includes('and'));
  return solution;
};
...
const oddValues = (arr) => {
  let solution = arr.filter((a) => a % 2 !== 0);
  return solution;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let difference = forbiddenValues
    .filter((x) => !arr.includes(x))
    .concat(arr.filter((x) => !forbiddenValues.includes(x)));
  return difference;
};
